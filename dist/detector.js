...
function checkSignalCooldown(lastSignal, currentType) {
  ...
  // ★ 优化1:缩短冷却时间,更快响应变盘行情
  var baseCooldown = 3 * 60 * 1000; // 基础冷却期从5分钟缩短到3分钟
  var sameDirectionCooldown = 7 * 60 * 1000; // 同方向信号冷却从10分钟缩短到7分钟
  var oppositeDirectionCooldown = 15 * 60 * 1000; // 相反方向信号冷却从20分钟缩短到15分钟
...
function calculateSignalStrength(signalType, conditions, resonance, bars) {
  ...
  // 基础条件评分 (0-17分) - 插针最重要,布林/MACD次之
  var condWeights = { '下影插针': 7, '上影插针': 7, '低点抬高': 5, '高点压制': 5, 'MACD配合': 5 };
  conditions.forEach(function(c) {
    var itemScore = c.ok ? (condWeights[c.label] || 5.7) : 0;
    score += itemScore;
    scoreDetails.baseConditions.score += itemScore;
    scoreDetails.baseConditions.items.push({
      label: c.label,
      score: Math.round(itemScore),
      max: condWeights[c.label] || 5.7,
      ok: c.ok,
      tip: c.tip
    });
  });
...
async function detectSignal(interval) {
  ...
  // ★ 优化3:信号质量加权评分(不再1/7平权)
  function calculateQualityScore(conditions) {
    var weights = {
      volume: 1.0,   // 成交量最重要,1倍权重
      rsi: 0.9,     // RSI次重要
      boll: 0.8,    // 布林带重要
      macd: 0.8,    // MACD重要
      ma: 0.6,      // MA均线一般重要
      trend: 0.7,   // 趋势方向重要
      reversal: 0.3 // 变盘信号权重最低
    };
    var totalWeight = Object.values(weights).reduce(function(a,b){return a+b;},0); // 4.1
    var weightedScore = 0;
    Object.keys(conditions).forEach(function(key) {
      if (conditions[key]) {
        weightedScore += weights[key] || 0.5;
      }
    });
  ...
  // ★ 优化4:趋势延续信号 - 放宽触发条件,不依赖插针形态
  // 模式1: 插针反转(原逻辑,最严格)
  // 模式2: 趋势延续(大涨行情核心!)- 强趋势+持续动能,不强制要求插针/c2Long
  // 模式3: 动量突破 - 短期快速拉升,MACD确认即可

  // 趋势延续做多:强上升趋势+动能持续+成交量配合(不需要插针!)
  var isTrendFollowLong = (
    trendDirection === 'up' && trendStrength >= 55 &&  // 强上升趋势
    momentum > 0.4 &&  // 有明确上涨动能
    volumeConditions.long  // 成交量放大
  );

  // 动量突破做多:只要趋势不差+MACD配合+当前K线强势就触发
  var isMomentumBreakoutLong = (
    momentum > 0.5 &&  // 短期涨幅>0.5%(从0.3提高,避免假信号)
    trendDirection !== 'down' &&  // 非下降趋势即可(从强趋势改为中性)
    (macdData.dif[n] > macdData.dea[n] || macdBar > 0) &&  // MACD确认
    lastBar.close >= lastBar.open  // 当前K线收阳
  );

  // ★ 优化4续:放宽插针触发条件 - 不再强制c2Long(趋势延续时c2Long可能不满足)
  var canLong = (isLongPin && c4Long && longQualityScore >= 55) ||  // 插针模式:去掉了c2Long
                 isTrendFollowLong ||  // 趋势延续模式
                 isMomentumBreakoutLong;  // 动量突破模式
  if (canLong) {
  ...
  // ★ 优化4续:做空条件与做多对称,同样放宽
  // 趋势延续做空:强下降趋势+动能持续+成交量配合(不需要插针!)
  var isTrendFollowShort = (
    trendDirection === 'down' && trendStrength >= 55 &&  // 强下降趋势
    momentum < -0.4 &&  // 有明确下跌动能
    volumeConditions.short  // 成交量放大
  );

  // 动量突破做空:只要趋势不差+MACD配合+当前K线弱势就触发
  var isMomentumBreakoutShort = (
    momentum < -0.5 &&  // 短期跌幅>0.5%
    trendDirection !== 'up' &&  // 非上升趋势即可
    (macdData.dif[n] < macdData.dea[n] || macdBar < 0) &&  // MACD确认
    lastBar.close <= lastBar.open  // 当前K线收阴
  );

  // ★ 优化4续:放宽插针触发条件 - 不再强制c2Short
  var canShort = (isShortPin && c4Short && shortQualityScore >= 55) ||  // 插针模式:去掉了c2Short
                 isTrendFollowShort ||  // 趋势延续模式
                 isMomentumBreakoutShort;  // 动量突破模式
  if (canShort) {
    // 检查信号冷却
    var lastSignal = getLastSignalInfo();
  ...
  // 横盘整理特殊处理:降低信号评分
    ...
    // ★ 优化2:横盘整理特殊处理 - 动量自适应降分,不再固定-8
    // 零动量横盘-6分,动量模糊横盘-3分,有明确动量就不降
    if (trendInfo.trend === 'sideways') {
      var absMom = Math.abs(momentum);
      var sidewaysReduction = 0;
      if (absMom < 0.1) {
        sidewaysReduction = 6;  // 几乎没动,完全横盘
      } else if (absMom < 0.3) {
        sidewaysReduction = 3;  // 轻微波动
      }
      // absMom >= 0.3: 横盘整理中有明确方向,不降分
  ...
  // 额外加入高周期的近期极值（关键价位，更大周期的支撑阻力更有参考价值）
  if (higherBars && higherBars.length > 5) {
    var hLookback = Math.min(higherBars.length, 20);
    for (var hpi = higherBars.length - hLookback + 2; hpi < higherBars.length - 1; hpi++) {
      if (higherBars[hpi].low <= higherBars[hpi-1].low && higherBars[hpi].low <= higherBars[hpi+1].low) {
        pivotPoints.supports.push(higherBars[hpi].low);
      }
      if (higherBars[hpi].high >= higherBars[hpi-1].high && higherBars[hpi].high >= higherBars[hpi+1].high) {
        pivotPoints.resistances.push(higherBars[hpi].high);
      }
    }
  }
  ...
  if (signalType === 'long') {
    ...
    if (trendDirection === 'up' && momentum >= 0) {
      // 上升趋势 + 正动量/零动量 → 偏多
      // ★ 优化6:上限收紧到70(不在正式信号60-100范围,避免混淆)
      var longScore = Math.min(70, Math.max(50, Math.round(trendStrength * 0.5 + Math.abs(momentum) * 8 + 35)));
      signalConfidence = longScore;
    } else if (trendDirection === 'down' && momentum <= 0) {
      // 下降趋势 + 负动量/零动量 → 偏空
      var shortScore = Math.min(70, Math.max(50, Math.round(trendStrength * 0.5 + Math.abs(momentum) * 8 + 35)));
      signalConfidence = -shortScore;
    } else if (trendDirection === 'sideways') {
      // 震荡市:用纯动量判断方向
      if (momentum > 0.15) {
        signalConfidence = Math.min(65, Math.max(45, Math.round(40 + Math.abs(momentum) * 10)));
