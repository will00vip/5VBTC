...
class AutoPushSystem {
  ...
  sendNotification(record) {
    ...
    const isLong = record.score > 0 || record.direction === '做多'
    const dirLabel = isStrong
      ? (isLong ? '📈 做多信号' : '📉 做空信号')
      : (isLong ? '📈 偏多提醒' : '📉 偏空提醒')
    const title = `${dirLabel} ${record.score}分`
    const content = `
...
function startApp() {
  // 启动诊断
  console.log('[Diag] CanvasChart:', !!window.CanvasChart, '| fetchKlines:', !!window.fetchKlines, '| LWChart:', !!window.LWChart, '| BTCSignal:', !!window.BTCSignal)
  const c = document.getElementById('lwChartContainer')
  if (c) console.log('[Diag] lwChartContainer:', c.offsetWidth, 'x', c.offsetHeight)
  else console.error('[Diag] lwChartContainer 不存在!')
...
async function fetchSignalData() {
  ...
  try {
    ...
    const detectFn = window.BTCSignal.detectBtcSignal || window.BTCSignal.detectSignal
...
function processSignalResult(result) {
  ...
  // ── 五维度评分卡 ──
  try { updateScoreCard(result) } catch(e) { console.warn('[scoreCard]', e) }
  ...
  // ── V10.1: 有信号类型即记录（>=60分即有type）─
  const isRealSignal = result.type != null
  if (isRealSignal) {
    // 重置等待时间
    resetCatWaitTimer()
  ...
  if (isRealSignal && score >= 85) {
    startCountdown(result)
  } else {
    // 检查是否有有效的缓存信号
    const cache = getValidSignalCache()
    if (cache && Math.abs(cache.signalConfidence) >= 85) {
...
function displaySignalInfo(result) {
  ...
  if (directionIcon && directionTextEl && result.type) {
    const isLong = result.type === 'long'
    const conf = Math.abs(result.signalConfidence || 0)
    // ★ v9.0: 85+强烈信号，60-84观察，<60无信号
    if (conf >= 85) {
      directionIcon.textContent = isLong ? '🟢' : '🔴'
      directionTextEl.textContent = isLong ? '做多信号' : '做空信号'
    } else if (conf >= 60) {
      directionIcon.textContent = isLong ? '📈' : '📉'
      directionTextEl.textContent = isLong ? '偏多提醒' : '偏空提醒'
    } else {
      directionIcon.textContent = '⚪'
      directionTextEl.textContent = '观察中'
    }
  }
...
// 更新信号缓存
// 更新信号缓存(兼容三步法0-12分制和detectSignal 100分制)
function updateSignalCache(result) {
  // ★ v9.0 直接使用 signalConfidence(已含正负号,100分制)
  const conf = result.signalConfidence || 0

  if (conf !== 0) {
    signalCache = {
      signalConfidence: conf,  // 统一存储为100分制
      type: result.type,
      timestamp: Date.now(),
      tradeLevels: result.tradeLevels,
...
function calculateOverallScore(result) {
  ...
  // ★ v9.0 直接使用新打分系统的 signalConfidence(已含正负号)
  if (result.signalConfidence !== undefined && result.signalConfidence !== 0) {
    updateSignalCache(result)
    return Math.round(result.signalConfidence)
  }

  // 缓存兜底
  const cache = getValidSignalCache()
  if (cache && cache.signalConfidence !== undefined && cache.signalConfidence !== 0) {
    return Math.round(cache.signalConfidence)
  }
...
function updateScoreDial(score, hasSignal = false, trend = 'neutral', signalResult = null) {
  try {
    ...
    // 优先用缓存(保留最近一次推送的分数,5分钟内有效)
    let displayScore = score
    let displayHasSignal = hasSignal
    if (scoreCache.pushScore !== 0 && now < scoreCache.validUntil) {
      // 只要缓存有效就用缓存(防止分数在刷新瞬间消失)
      displayScore = scoreCache.pushScore
      displayHasSignal = true
    }

    const absSignalScore = Math.abs(displayScore)
    const isLong = displayScore > 0

    // ★ 三档显示逻辑
    if (absSignalScore >= 85) {
      // 85+:做多/做空信号
      const icon = isLong ? '💚' : '❤️'
      const label = isLong ? '做多信号' : '做空信号'
      bubbleTitle.textContent = icon + ' ' + label
      bubbleText.textContent = `${label} 🐂🐻\n${absSignalScore}分 | 3分钟倒计时中...`
      if (statusContent) statusContent.className = 'status-content ' + (isLong ? 'long' : 'short')

      if (window.showSignalCountdown) {
        window.showSignalCountdown(displayScore, 3 * 60 * 1000)
      }
    } else if (absSignalScore >= 60) {
      // 60-84:偏多/偏空提醒
      const icon = isLong ? '📈' : '📉'
      const label = isLong ? '偏多提醒' : '偏空提醒'
      bubbleTitle.textContent = icon + ' ' + label
      bubbleText.textContent = `${label}\n${absSignalScore}分 | 等待更确认信号`
      if (statusContent) statusContent.className = 'status-content ' + (isLong ? 'long' : 'short')
      ...
      // <60:观察中
      let statusLabel = '🔵 观察中'
      let displayText = '等待信号中...'

      // 处理不同的趋势值
      let normalizedTrend = trend
      if (trend === 'strong_bull' || trend === 'bull') normalizedTrend = 'up'
      else if (trend === 'strong_bear' || trend === 'bear') normalizedTrend = 'down'

      if (normalizedTrend === 'up') {
        statusLabel = '💚 偏多观察'
        displayText = '行情偏多,等待信号确认'
      } else if (normalizedTrend === 'down') {
        statusLabel = '❤️ 偏空观察'
        displayText = '行情偏空,等待信号确认'
      }

      bubbleTitle.textContent = statusLabel
      bubbleText.textContent = displayText
      if (statusContent) statusContent.className = 'status-content warning'
...
// 导出到window供其他模块调用
window.updateScoreDial = updateScoreDial; window.updateScoreCard = updateScoreCard

// 五维度双向评分卡
function updateScoreCard(result) {
  var cb = document.getElementById("threeStepCard");
  if (!cb) return;
  var sb = result.scoreBreakdown || {};
  var ld = sb.long || {};
  var sd = sb.short || {};
  var abs = Math.abs(result.score || 0);
  var isLong = result.type === "long";
  var strong = abs >= 85;
  var ss = result.signalConfidence > 0 ? "+" + result.signalConfidence : result.signalConfidence;
  var bc = strong ? "badge-fire" : abs >= 60 ? "badge-warn" : "badge-wait";
  var bt = strong ? "🔥 信号触发" : abs >= 60 ? "⚡ 偏多/偏空" : "⏸ 观察中";
  var ds = ["基础条件", "多周期共振", "趋势强度", "波动率", "成交量"];
  var ks = ["dim1", "dim2", "dim3", "dim4", "dim5"];
  var mx = [17, 43, 17, 17, 6];
  var rows = ds.map(function(l, i) {
    var lv = ld[ks[i]] || 0;
    var sv = sd[ks[i]] || 0;
    var v = isLong ? lv : sv;
    var pct = Math.round(v / mx[i] * 100);
    var bw = Math.max(5, pct);
    var color = isLong ? "#10b981" : "#ef4444";
    return "<div class=dim-row><span class=dim-label>" + l + "</span><div style=background:#1f2937;padding:2px;border-radius:4px><div style=width:" + bw + "%;background:" + color + ";height:8px;border-radius:4px></div></div><span class=dim-score>" + v + "/" + mx[i] + "</span></div>";
  }).join("");
  var hint = strong ? "🔥 强烈信号！" : abs >= 60 ? "💡 等待确认" : "⏸ 低于阈值";
  cb.innerHTML = "<div class=three-step-header><span class=three-step-title>⚡ 五维度评分</span><span class=three-step-badge " + bc + ">" + bt + "</span></div><div class=score-display>" + ss + "分</div><div>" + rows + "</div><div class=score-hint>" + hint + "</div>";
}
// 自动保存信号记录
...
function _pushNotification(result) {
  try {
    ...
    // ★ V10.1: 推送85分以上信号（60-84仅UI显示不推送）
    if (absScore < 85) {
      console.log('[Push] 分数低于85，不推送:', score)
      return
    }

    // 横盘整理时不推送
    if (result.trend === 'sideways') {
      console.log('[Push] 横盘整理不推送:', score)
      return
    }

    // V10.1: 方向文本 - 85+=做多/做空信号🔥, 60-84=偏多/偏空提醒
    const isLong = score > 0
    const isStrong = absScore >= 85
    const directionText = isStrong
      ? (isLong ? '📈 做多信号' : '📉 做空信号')
      : (isLong ? '📈 偏多提醒' : '📉 偏空提醒')
    const qualityTag = isStrong ? '🔥强烈' : '⚡提示'
    const specialHint = isStrong ? '\n🚨 强烈信号！' : '\n💡 等待确认中'
    const scoreStars = '⭐'.repeat(Math.min(result.signalStrength || 5, 5))
    const price = result.bars ? result.bars[result.bars.length - 1].close : '--'

    // ── 趋势方向提示 ──
    const trendTag = result.trendWarning || ''
    // ── 60-69分专属警示 ──
    const weakWarning = (absScore >= 60 && absScore < 70) && result.type ? '\n⚠️ 评分偏低建议等待确认' : (absScore >= 70 && absScore < 85 ? '\n💡 接近强烈信号' : '')
...
function updateSignalInfo(result) {
  ...
  // 方向信息
  const isLong = result.type === 'long'
  const conf = Math.abs(result.signalConfidence || result.signalStrength || 0)
  const strong = conf >= 85
  setContent('directionIcon', strong ? (isLong ? '🟢' : '🔴') : (isLong ? '📈' : '📉'))
  setContent('directionText', strong ? (isLong ? '做多信号' : '做空信号') : (isLong ? '偏多提醒' : '偏空提醒'))
...
function _openRealSimPos(result, price) {
  ...
  const absScore = Math.abs(score)
  if (absScore < 85) {
    console.log('[模拟交易] 分数低于85，不开仓:', score)
    return
  }
...
function updateSignalIndicator() {
  ...
  if (score >= 85) {
    ...
    if (textEl) { textEl.textContent = '做多信号 (' + score + '分)'; textEl.style.color = '#ef4444' }
  } else if (score >= 60) {
    if (dotEl) dotEl.className = 'signal-dot buy'
    if (textEl) { textEl.textContent = '偏多提醒 (' + score + '分)'; textEl.style.color = '#34d399' }
  } else if (score <= -85) {
    if (dotEl) dotEl.className = 'signal-dot sell'
    if (textEl) { textEl.textContent = '做空信号 (' + Math.abs(score) + '分)'; textEl.style.color = '#22c55e' }
  } else if (score <= -60) {
    if (dotEl) dotEl.className = 'signal-dot sell'
    if (textEl) { textEl.textContent = '偏空提醒 (' + Math.abs(score) + '分)'; textEl.style.color = '#f87171' }
  } else {
    if (dotEl) dotEl.className = 'signal-dot waiting'
    if (textEl) { textEl.textContent = '等待信号...'; textEl.style.color = '#94a3b8' }
  }
}
...
const AutoTrade = {
  ...
  showTradeNotification(type, direction, price, leverage, margin, score, support, resistance) {
    ...
    if (absScore >= 85 && typeof Android !== 'undefined' && Android.showNotification) {
      const label = absScore >= 85
        ? (direction === 'long' ? '🟢 做多信号' : '🔴 做空信号')
        : (direction === 'long' ? '📈 偏多提醒' : '📉 偏空提醒')
      const title = `${label}`
  ...
  // 判断是否应该开仓（固定85分阈值，不从UI读取）
  shouldOpen(score, direction) {
    const absScore = Math.abs(score)
    // ★ v9.1 固定85分阈值
    if (direction === 'long' && score >= 85) return true
    if (direction === 'short' && absScore >= 85) return true
    return false
  },
  ...
  sendNotification(direction, score, price, support, resistance) {
    if (typeof Android !== 'undefined' && Android.showNotification) {
      ...
      if (absScore < 85) return  // ★ v9.0 只推送85分以上
      const isLong = direction === 'long'
      const title = isLong ? '📈 做多信号' : '📉 做空信号'
      const body = `${title} - ${absScore}分 | ${isLong?'30x':'30x'}杠杆\n价格: $${price.toFixed(0)} | 支撑: ${support ? support.toFixed(2) : '无'} | 阻力: ${resistance ? resistance.toFixed(2) : '无'}`
      Android.showNotification(title, body)
    }
